# 🧑‍💻 Ye Xiaonan

### Full-stack Developer · Network Engineer 

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=auto&height=140&text=Hello%20World&fontSize=38&fontColor=ffffff&animation=fadeIn&fontAlignY=32" />
</div>

<div align="center" style="max-width:600px; font-size:15px;">
专注后端开发、分布式架构、AI 本地推理部署，以及网络系统设计。  
偏好高可靠架构、极简代码和优雅工程实践。  
</div>

---

## 🍎 Core Skills

<div align="center">

<table style="width:85%; min-width:280px;">
<tr>
<td align="center">

**Backend Development**
Java · Go · Python
Spring Boot · Spring Cloud · Gin

</td>
<td align="center">

**Systems & Cloud**
Linux · Docker · Kubernetes
Nginx · CI/CD

</td>
</tr>
</table>

</div>

---

## 🧭 GitHub Overview

<div align="center">

<img height="150" src="https://github-readme-stats-evirunurm.vercel.app/api?username=yexiaonan&show_icons=true&theme=transparent&hide_border=true" />

<img height="150" src="https://github-readme-stats-evirunurm.vercel.app/api/top-langs/?username=yexiaonan&layout=compact&theme=transparent&hide_border=true" />

</div>


---

## 🌤 Activity

<div align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=yexiaonan&theme=github-light&area=true&hide_border=true" />
</div>

---

## 🏆 Awards

<div align="center">

**2025 世界职业院校技能大赛 · 金奖**
**2025 区级技能大赛 · 金奖**
**2024 全国职业技能大赛 · 三等奖**

</div>

---

## ✉️ Contact

<div align="center">

**Email：** [yexiaonanair@outlook.com](mailto:yexiaonanair@outlook.com)

</div>

---

## ✨ Quote

<div align="center">

<img src="https://quotes-github-readme.vercel.app/api?type=vertical&theme=light&quote=计算，为了无法计算的时间。&author=叶小楠"/>

</div>

